import {
    TLBlockDeserializeResultType,
    TLBlockSerializerV1,
} from "@super-protocol/tee-lib";
...
import { Encryption, Resource, UrlResource } from "@super-protocol/sp-dto-js";
...
class TIIGenerator {
    public static async generateByOffer(
        ...
        return JSON.stringify({
            ...
            tri: await Crypto.encrypt(
                "ECIES",
                JSON.stringify(tri),
                tlb.data.teePubKeyData.toString("hex")
            ),
        });
    }
    ...
    public static async getTRI(
        tii: string,
        encryptionKey: string,
        algo: CryptoAlgorithm
    ): Promise<TeeRunInfo> {
        const tiiObj = JSON.parse(tii);
        const tri: string = await Crypto.decrypt(
            algo,
            tiiObj.tri,
            encryptionKey
        );
    ...
    public static async getUrl(
        ...
        const res = await TIIGenerator.getResource<UrlResource>(tii, key, algo);
        return res.url;
    }
    ...
    public static async getResource<T>(
        ...
        const tiiObj = JSON.parse(tii);
        const resource: string = await Crypto.decrypt(
            algo,
            tiiObj.encryptedResource,
            key
        );
